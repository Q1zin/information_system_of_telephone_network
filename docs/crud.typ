...
#table(
  ...
  [Триггер (`RAISE EXCEPTION`)], [400], [Создать 2-го абонента на основном номере → «К номеру № 1 (тип линии «основной») можно подключить не более 1 абонент(ов)»],
...
Все сообщения — на русском: статические ошибки и коды PostgreSQL переводятся в `error.rs` (например `23505`/`UNIQUE` → «Запись с такими данными уже существует»), а тексты триггеров заданы по-русски в самой БД. Поэтому форма на фронте показывает абоненту/оператору понятную причину отказа, а данные в БД остаются корректными при любом раскладе.
...
#term[Поля-ссылки вместо ручного ввода id]. Где раньше в форме вводили число (`pbx_id`, `address_id`, `subscriber_id` …), теперь тип поля `reference`: `CrudView` подгружает связанный справочник и показывает #term[выпадающий список] с поиском и человекочитаемой подписью («#12 · Иванов Иван»), а в форму уходит выбранный `id`. Те же подписи используются и в #term[таблице]: вместо «голых» id выводятся названия, enum-значения — русскими (`active` → «Активен»), а булевы — «Да/Нет». Так оператор не угадывает числа, а выбирает из списка.
