...
#align(center)[
  ...
  #v(0.5cm)
  #text(size: 11pt)[Что делает каждый файл `backend/migrations/0001…0014` — простыми словами]
  #v(0.8cm)
...
#term[Миграции] — это пронумерованные SQL-файлы (`0001_…`, `0002_…`, … `0014_…`), которые #term[по очереди] строят базу данных с нуля: создают типы, таблицы, связи, ограничения, триггеры, представления и заливают начальные данные. Порядок важен: нельзя сослаться на таблицу, которой ещё нет, поэтому, например, типы (`0001`) идут раньше таблиц, а таблицы — раньше триггеров.
...
#creates[8 функций (включая 2 для русских подписей enum) и 7 триггеров, которые автоматически проверяют сложные правила при изменении данных.]
...
- `ent || ':' || act` — склейка строк: получаются коды `pbx:read`, `pbx:create`, … (15 сущностей × 4 действия = 60 прав) + спец-права `analytics:read`, `raw_query:run`, `rbac:manage`. Человекочитаемое описание к каждому праву собирается по-русски функциями `perm_action_ru()` / `perm_entity_ru()` (`pbx:read` → «Просмотр — АТС»).
...
= 0013 — русские тексты триггеров (донакат)

#creates[Переопределяет функции триггеров из `0008`, чтобы сообщения об ошибках были на русском — #term[без] пересоздания базы.]

Тексты ошибок из триггеров (`RAISE EXCEPTION`) видит оператор прямо на сайте, поэтому они должны быть понятными и на русском. Для #term[новых] баз тексты уже исправлены в `0008`, но база, созданная #term[раньше], это изменение «не увидит»: применённые миграции повторно не запускаются. Поэтому правка оформлена #term[отдельным] файлом.

```sql
CREATE OR REPLACE FUNCTION trg_number_intercity_check() RETURNS trigger AS $$
... RAISE EXCEPTION 'Для номера городской АТС % межгород должен быть открыт или закрыт ...' ...
$$ LANGUAGE plpgsql;
```

- `CREATE OR REPLACE FUNCTION` — заменяет #term[тело] функции, не трогая таблицы и сами триггеры. Триггеры ссылаются на функцию по имени, поэтому после замены сразу работают по-новому.
- Файл #term[идемпотентен]: его можно накатить на уже работающую базу (`psql -f …/0013_triggers_ru.sql`) — он лишь обновит функции.
- Заодно добавлены две подписи-помощника `pbx_type_ru()` / `line_type_ru()`: переводят значения enum внутри сообщений (`city` → «городская», `main` → «основной»), чтобы в тексте не светилась латиница.

#note[Это #term[правильный приём] изменения уже применённой БД: не редактировать старый файл «задним числом», а добавить новый с `CREATE OR REPLACE` / `UPDATE`.]

= 0014 — русские описания ролей и прав (донакат)

#creates[Переводит на русский описания ролей и прав, заведённые в `0010`, в #term[уже существующей] базе.]

В админке (раздел «Роли») у каждой роли показывается описание; в `0010` они были на английском («Full access to everything», «Manage telephone-network data, run analytics \& raw queries»). Этот файл обновляет их и в новых, и в старых базах.

```sql
-- описание прав собирается из кода: 'pbx:read' → 'Просмотр — АТС'
UPDATE permission SET description =
    perm_action_ru(split_part(code, ':', 2)) || ' — ' || perm_entity_ru(split_part(code, ':', 1))
 WHERE code LIKE '%:%' AND code NOT IN ('analytics:read','raw_query:run','rbac:manage');
UPDATE role SET description = 'Полный доступ ко всему' WHERE name = 'superadmin';
```

- `UPDATE … WHERE …` правит #term[существующие] строки — в отличие от `INSERT … ON CONFLICT DO NOTHING` в `0010`, который существующие строки не трогает (поэтому донакат и нужен).
- `split_part(code, ':', 1/2)` — разбирает код права на «сущность» и «действие».
- Тоже идемпотентен: повторный запуск просто перезапишет те же тексты.
